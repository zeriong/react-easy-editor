...
import type { SerializedTableRowNode as SerializedBaseTableRowNode } from "@lexical/table";
...
export class StyledTableRowNode extends TableRowNode {
  ...
  setHeight(height?: number): this {
    if (height != null && Number.isFinite(height)) {
      const v = `${Math.max(0, Math.round(height))}px`;
      return this.setStyle(setStyleProp(this.getStyle(), "height", v));
    }
    return this.setStyle(setStyleProp(this.getStyle(), "height", null));
  }
  ...
  updateDOM(prev: StyledTableRowNode): boolean {
    return normalizeRowStyle(prev.__style) !== normalizeRowStyle(this.__style);
  }
