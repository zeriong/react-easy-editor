...
export class StyledTableCellNode extends TableCellNode {
  ...
  constructor(isHeader: boolean, colSpan?: number, rowSpan?: number, style = "", key?: NodeKey) {
    const headerState = isHeader ? TableCellHeaderStates.ROW : TableCellHeaderStates.NO_STATUS;
    super(headerState, colSpan, undefined, key);
  ...
  updateDOM(prev: StyledTableCellNode): boolean {
    if (!!prev.__isHeader !== !!this.__isHeader) return true;
    if (prev.__style !== this.__style) return true;
    if ((prev.__colSpan ?? 1) !== (this.__colSpan ?? 1)) return true;
    if ((prev.__rowSpan ?? 1) !== (this.__rowSpan ?? 1)) return true;
    return false;
  }

  static importJSON(json: SerializedStyledTableCellNode): StyledTableCellNode {
    const isHeader = !!json.isHeader;
