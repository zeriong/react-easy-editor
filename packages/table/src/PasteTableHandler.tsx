...
import { parseCssRules, mergeStyles } from "@react-easy-editor/core";
...
function applyParagraphAlign(paragraph: ParagraphNode, styleOrFallback: string): void {
  ...
  if (align) {
    try {
      paragraph.setFormat(align as Parameters<ParagraphNode["setFormat"]>[0]);
    } catch { /* empty */ }
  }
}
...
function buildParagraphsFromCellDOM(
  ...
  const flush = () => {
    if (curP.getChildrenSize() > 0) {
      if (!pickTextAlign((curP as unknown as { getStyle?: () => string }).getStyle?.() || "") && cellStyleForFallback) {
...
/* ------------------------------------------------------------------ */
/*  Inline line-break insert helper                                    */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/*  PasteTableHandler                                                  */
/*  Registers PASTE_COMMAND to intercept table HTML / TSV from         */
/*  clipboard (e.g. Excel, Google Sheets) before image handler.        */
/* ------------------------------------------------------------------ */

export function PasteTableHandler({ editor }: { editor: LexicalEditor }) {
  useEffect(() => {
    const unregister = editor.registerCommand(
      ...
      (e: ClipboardEvent) => {
        ...
        // ---------- Case 1: HTML with <table> (Excel, Google Sheets) ----------
        ...
        // ---------- Case 2: TSV (Tab-Separated Values) ----------
