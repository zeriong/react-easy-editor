...
/* ------------------------------------------------------------------ */
/*  Command: dispatched when the user finishes resizing an image       */
/* ------------------------------------------------------------------ */
...
const HANDLE_POINT_CLASS = "ree-image-handle-point";
...
export function ImageComponent({
  ...
  useEffect(() => {
    ...
    // assign a unique id to the image element
    const newId = `ree_img_${Date.now()}_${Math.random()}`;
    imgIdRef.current = newId;
    if (imgRef.current) imgRef.current.id = newId;
  ...
  /* -- compute initial size from the natural image dimensions -- */
      ...
      .finally(() => {
        ...
        if (!width && !height) {
          ...
          initHeight = ratio
            ...
            : Math.max(1, Math.round(initWidth * 0.666));
        }
  ...
  return (
    ...
    >
      <img ref={imgRef} src={src} alt={alt} style={styles.img} draggable={false} />
